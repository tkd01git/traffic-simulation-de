...
# 編集対象ファイルのリストを作成
file_indices = ["1","2","3","4","5"] # 必要に応じて変更
input_files = [f"jamnodata{x}.xlsx" for x in file_indices]
...
# シートにランキングデータを書き込む関数
def write_sheet_with_global_ranks(writer, sheet_name, time_data, speed_data, dens_data, flow_sum, global_ranks):
    output_data = {
        'Time': list(time_data),
        'speed': list(speed_data),
        'Density': list(dens_data),
...
for file_index, input_file in zip(file_indices, input_files):
    result_file1 = f"jamnoresult{file_index}.xlsx"
    result_file2 = f"jamnoλrankresult{file_index}.xlsx"
    ...
    df2 = pd.read_excel(input_file, sheet_name=sheet_name2, usecols=f'{start_col}:{end_col}', header=None)

    data_frames = [
        pd.read_excel(input_file, sheet_name=sheet_name1, usecols=f'{start_col}:{end_col}', header=None, skiprows=i, nrows=20)
    ...
    with pd.ExcelWriter(result_file2, mode='w') as writer:
        for i, (df, sheet_name) in enumerate(zip(
            ...
            write_sheet_with_global_ranks(writer, sheet_name, time_data, speed_data, dens_data, flow_sum, global_ranks)
