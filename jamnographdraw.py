...
file_names = [f"jamnoresult{num}.xlsx" for num in file_numbers]
...
for col_idx, file_name in enumerate(file_names):
    ...
    file_number = int(file_name.replace('jamnoresult', '').replace('.xlsx', ''))
    ...
    ax.set_title(f'File {file_number} - Speed', fontsize=10)
    ax.legend()
    ax.set_ylim(0, 100) 
    ax.grid()
...
# 全体のタイトルを追加
fig.suptitle(" 3sdata : F_lambda2", fontsize=16)
