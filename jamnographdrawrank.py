...
file_names = [f"jamnoλrankresult{num}.xlsx" for num in file_numbers]
...
for col_idx, file_name in enumerate(file_names):
    ...
    # ファイル番号を抽出
    file_number = int(file_name.replace('jamnoλrankresult', '').replace('.xlsx', ''))
...
# 全体のタイトルを追加
fig.suptitle("3sdata : rank of F_lambda2", fontsize=16)
