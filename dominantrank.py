...
# シートにデータを書き込む関数
def write_sheet_with_ranks(writer, sheet_name, time_data, flow_data, dens_data, flow_sum, global_ranks):
...
with pd.ExcelWriter(output_file, mode='w') as writer:
    for i, (df, sheet_name) in enumerate(zip(
        ...
        flow_sum = (
            pd.Series(flow_data)
            .rolling(window=19, min_periods=1, center=True)
            .sum()
        )

        # シートにデータを書き込み
        write_sheet_with_ranks(writer, sheet_name, time_data, flow_data, dens_data, flow_sum, global_ranks)
